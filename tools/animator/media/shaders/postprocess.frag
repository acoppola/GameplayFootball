...
//  return;
  ...
  //SSAO = pow(SSAO, 2.0f);
  //SSAO = texture(map_modifier, texCoord).g;
  //SSAO = clamp(SSAO * 3.0f - 2.0f, 0.0f, 1.0f);
  //if (texCoord.x > 0.5) SSAO = 1;
...
//  vec3 fogColor = vec3(1.0, 0.9, 0.86);
  vec3 fogColor = vec3(0.85, 0.6, 1.0);

  //float fogFactor = clamp(fragDepth * 0.0018 - 0.1, 0.0, 0.08); // tele cam
  //float fogFactor = clamp(fragDepth * 0.0012 - 0.08, 0.0, 0.07); // tele cam
  //float fogFactor = clamp(fragDepth * 0.0011 - 0.05, 0.0, 0.06); // tele cam
  //float fogFactor = clamp(fragDepth * 0.002 * fogScale - 0.1 * fogScale, 0.0f, 0.12f);
  float fogFactor = clamp(fragDepth * 0.007f * (1.0f - fogScale) - 0.16f * fogScale, 0.0f, 0.25f);

  fragColor = fragColor * (1.0f - fogFactor) + fogColor * fogFactor;
  if (depth > 0.999) fragColor = fogColor; // fill 'background'/sky
  ...
  */
  ...
  float contrastBias = 0.0f;//0.3f; // 0 == normal .. 1 == 'fake hdri'
  float saturation = 1.0f * (0.4f + SSAO * 0.6f); // SSAO shadows are less saturated

  //finalBrightness = 1.0f;//1.8;// + clamp(-brightness + 0.5, -0.5, 0.5);// / clamp(brightness * 8.0 - 3.0, 0.2, 1.8);// clamp((3.0 - brightness * 4.0), 0.5, 1.5);
  //contrast = 1.0f;
  //gamma = 1.0f;//2.2f;
  //saturation = 1.0f;
  ...
  // now happens automagically because of glEnable(GL_FRAMEBUFFER_SRGB) (edit, nope, do it manually)
/*
...
*/
  ...
  /*
  fragColor.r = pow(fragColor.r, 1.0f / brightness);
  fragColor.g = pow(fragColor.g, 1.0f / brightness);
  fragColor.b = pow(fragColor.b, 1.0f / brightness);
  */
  ...
  /* visualize overflow
  if (fragColor.r > 1.0) fragColor.r = 0.0;
  if (fragColor.g > 1.0) fragColor.g = 0.0;
  if (fragColor.b > 1.0) fragColor.b = 0.0;
  */
  //fragColor = Compress(fragColor, 0.9f, 1.4f);
  fragColor = clamp(fragColor, 0.0, 1.0);

  // experiment: color bands/ranges
  // fragColor.r = 0.4f + fragColor.r * 0.6f;
  // fragColor.g = 0.1f + fragColor.g * 0.6f;
  // fragColor.b = 0.0f + fragColor.b * 0.6f;

  gl_FragColor = vec4(fragColor, 0);

  //SSAO = texture2D(map_modifier, texCoord).g;
  //gl_FragColor = vec4(vec3(SSAO), 0);
}
